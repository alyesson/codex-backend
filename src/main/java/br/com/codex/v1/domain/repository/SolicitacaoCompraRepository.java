...
public interface SolicitacaoCompraRepository extends JpaRepository<SolicitacaoCompra, Long> {
    ...
    @Query("SELECT s FROM SolicitacaoCompra s WHERE YEAR(s.dataSolicitacao) = :ano")
    List<SolicitacaoCompra> findAllByYear(@Param("ano") Integer ano);
    ...
    List<SolicitacaoCompra> findAllSolicitacaoPeriodo(@Param("dataInicial") Date dataInicial, @Param("dataFinal") Date dataFinal);
}
